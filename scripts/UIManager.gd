...
var player_honor_points: int:
	get:
		return alliance_honor
	set(value):
		alliance_honor = value
...
# --- INSTANTIATED SUB-MANAGERS ---
var save_manager
var permission_manager
var alliance_manager
var mail_manager
var chat_manager
...
func _ready() -> void:
	# Preload and instantiate all 8 decoupled sub-managers
	save_manager = preload("res://scripts/AllianceSaveManager.gd").new()
	save_manager.ui_manager = self
	
	permission_manager = preload("res://scripts/AlliancePermissionManager.gd").new()
	permission_manager.ui_manager = self
	
	alliance_manager = preload("res://scripts/AllianceManager.gd").new()
	alliance_manager.ui_manager = self
	
	mail_manager = preload("res://scripts/AllianceMailManager.gd").new()
	mail_manager.ui_manager = self
	
	chat_manager = preload("res://scripts/AllianceChatManager.gd").new()
	chat_manager.ui_manager = self
	
	research_manager = preload("res://scripts/AllianceResearchManager.gd").new()
	research_manager.ui_manager = self
	
	store_manager = preload("res://scripts/AllianceStoreManager.gd").new()
	store_manager.ui_manager = self
	
	application_manager = preload("res://scripts/AllianceApplicationManager.gd").new()
	application_manager.ui_manager = self
...
func _load_buildings_data() -> void:
	...
	# Fail-safe mock data matching standard scheme if file missing/empty
...
func _load_alliance_databases() -> void:
	...
	# Fallback initialization if files are missing or empty
		...
		alliances_db = [
			{
				...
				"help_requests": [
					{
						"id": "req_1",
						"player_name": "Vassal Gwydion",
						"type": "construction",
						"target": "Keep Level 12 Upgrade",
						"current": 14,
						"max": 30
					},
					{
						"id": "req_2",
						"player_name": "Lady Helen",
						"type": "research",
						"target": "Alchemical Metallurgy V",
						"current": 5,
						"max": 20
					}
				],
				...
				"members": [
					{"name": "Sovereign Maegan", "rank": 5, "power": 750000, "is_online": true, "last_online": "Now"},
					{"name": "Vassal Gwydion", "rank": 4, "power": 520000, "is_online": true, "last_online": "Now"},
					{"name": "Lady Helen", "rank": 3, "power": 410000, "is_online": false, "last_online": "3h ago"},
					{"name": "Sir Cedric", "rank": 2, "power": 320000, "is_online": true, "last_online": "Now"}
				],
				"rallies": [],
				"active_wars": [],
				"circulars": [
					{
						...
						"time": "1d ago",
						"message": "Welcome to Emerald Vanguard! Please donate daily to assist with our technology progression."
					}
				],
				"applicants": [],
				"invitations": []
	...
	# Guarantee all loaded alliances have the full set of required keys
	for alliance in alliances_db:
		if not alliance.has("circulars"):
			alliance["circulars"] = [
				{
					"title": "Welcome Sovereign!",
					"sender": alliance.get("leader", "Leader"),
					"time": "1d ago",
					"message": "Welcome to our Alliance! Please donate daily to assist with our technology progression."
				}
			]
		if not alliance.has("applicants"):
			alliance["applicants"] = []
		if not alliance.has("invitations"):
			alliance["invitations"] = []
		if not alliance.has("language"):
			alliance["language"] = "English"
		if not alliance.has("min_power"):
			alliance["min_power"] = 10000
		if not alliance.has("is_public"):
			alliance["is_public"] = true
		if not alliance.has("help_requests"):
			alliance["help_requests"] = []
		if not alliance.has("gifts"):
			alliance["gifts"] = []
		if not alliance.has("logs"):
			alliance["logs"] = []
		if not alliance.has("members"):
			alliance["members"] = []
		if not alliance.has("rallies"):
			alliance["rallies"] = []
		if not alliance.has("active_wars"):
			alliance["active_wars"] = []
	...
	# Add mock initial messages if empty
	if alliance_chat_messages.is_empty():
...
func leave_alliance() -> void:
	if alliance_manager:
		alliance_manager.leave_alliance()

func join_alliance(alliance_id: String) -> bool:
	if alliance_manager:
		return alliance_manager.join_alliance(alliance_id)
	return false

func accept_application(applicant_name: String) -> void:
	if application_manager:
		application_manager.accept_application(applicant_name)

func reject_application(applicant_name: String) -> void:
	if application_manager:
		application_manager.reject_application(applicant_name)
...
func create_alliance(alliance_name_str: String, tag_str: String, flag_symbol: String, description: String = "", banner_color: String = "", flag_color: String = "", is_public: bool = true, min_power: int = 10000, language: String = "English") -> bool:
	if alliance_manager:
		return alliance_manager.create_alliance(alliance_name_str, tag_str, flag_symbol, description, banner_color, flag_color, is_public, min_power, language)
	return false

func kick_alliance_member(member_name: String) -> void:
	if alliance_manager:
		alliance_manager.kick_alliance_member(member_name)

func promote_alliance_member(member_name: String) -> void:
	if alliance_manager:
		alliance_manager.promote_alliance_member(member_name)

func demote_alliance_member(member_name: String) -> void:
	if alliance_manager:
		alliance_manager.demote_alliance_member(member_name)

func donate_to_alliance_tech(tech_id: String, amount: int) -> bool:
	if research_manager:
		return research_manager.donate_to_alliance_tech(tech_id, amount)
	return false
...
func send_alliance_chat_message(text: String) -> void:
	if chat_manager:
		chat_manager.send_chat_message(text)
...
func _save_alliance_databases() -> void:
	if save_manager:
		save_manager.save_all()
	else:
		# Save alliance database to file fallback
		var file = FileAccess.open("res://data/alliance.json", FileAccess.WRITE)
		if file:
			file.store_string(JSON.stringify(alliances_db, "\t"))
			file.close()
